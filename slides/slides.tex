...
\begin{frame}{3 \;\; Architecture}
  ...
  \begin{tikzpicture}[font=\sffamily\small, >=Stealth, node distance=0.5cm,
    ...
    \draw[->, thick] (o) to[out=90,in=90,looseness=0.5] node[above,font=\footnotesize]{commit / route updates COP} (c);
    \node[draw, rounded corners, fill=gray!14, below=0.8cm of t, text width=15.5cm, align=center, xshift=2.2cm]
  ...
  \end{tikzpicture}}
  \vspace{0.3em}\\
  \footnotesize Each tick: world advances $\rightarrow$ COP refreshes $\rightarrow$ triage $\rightarrow$
  auction $\rightarrow$ award (through the HITL gate) $\rightarrow$ assets move along risk-aware routes,
  \emph{validated against the true flood at arrival}. The control plane is deterministic; only the world
  is stochastic (one seed).
\end{frame}
...
\begin{frame}{14 \;\; The artifact --- reproducible end to end}
  ...
  \begin{columns}[T]
    ...
    \textbf{Engineering}
    \begin{itemize}\footnotesize
      \item 39 modules, acyclic DAG, complexity $\le 10$
      \item 69 behaviour/invariant tests, 97\% coverage
      \item ruff + mypy + markdownlint clean; live loop deterministic
      \item optional offline LLM perception layer --- opt-in, guardrailed, off by default
    \end{itemize}
    \textbf{Run it}
    \begin{itemize}\footnotesize
