...
loss) are separate so injecting stress never perturbs the underlying disaster. The live coordination
loop uses no LLM by design (a deliberate reading of the governance requirement for deterministic,
auditable life-safety decisions); all results reported here use the default configuration, which
needs no API key. An optional, opt-in, offline-by-default advisory LLM perception layer exists but is
disabled throughout, and is bounded by an output guardrail that re-validates any extraction against
ground truth---so it can never alter a safety-relevant value. Evaluation is two-tier:
